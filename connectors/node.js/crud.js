  ...
  if (err) {
    ...
    async.waterfall([
      ...
      function Insert(name, location, callback) {
        ...
        request = new Request(
          'INSERT INTO Company.Employees (Name, Location) OUTPUT INSERTED.Id VALUES (@Name, @Location);',
      ...
      function Update(name, location, callback) {
        ...
        request = new Request(
          'UPDATE Company.Employees SET Location=@Location WHERE Name = @Name;',
          function(err, rowCount, rows) {
            if (err) {
              callback(err);
            } else {
      ...
      function Delete(name, callback) {
        ...
        request = new Request(
          'DELETE FROM Company.Employees WHERE Name = @Name;',
          function(err, rowCount, rows) {
            if (err) {
              callback(err);
            } else {
      ...
      function Read(callback) {
        ...
        request = new Request(
        'SELECT Id, Name, Location FROM Company.Employees;',
        function(err, rowCount, rows) {
          if (err) {
            callback(err);
          } else {
